...
function ThemeButton() {
  ...
  const systemIcon = <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
  ...
  const [theme, setTheme] = useState(window.localStorage.getItem("theme") ||  systemTheme());
   ...
   <DropdownMenu.Root>
    ...
    <DropdownMenu.Portal>
      <DropdownMenu.Content className='px-4 py-2  font-semibold w-auto h-auto rounded-lg ring-1 flex flex-col gap-2 dark:text-white dark:bg-slate-800 dark:ring-0 dark:highlight-white/5'>
        {options.map(option =>
          <DropdownMenu.Item className='inline-block hover:outline-none hover:dark:bg-gray-700 hover:cursor-pointer' onClick={() => {
...
export default function Layout({ children }: { children: ComponentChildren }) {
    ...
    return (
        <div className="flex h-screen flex-col bg-gradient-to-b px-4 pt-4 dark:from-[#2e026d] dark:to-[#15162c] dark:text-white">
            <Navbar />
